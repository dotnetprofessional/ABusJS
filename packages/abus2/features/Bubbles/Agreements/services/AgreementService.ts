...
export class AgreementService {
    ...
    public async getAgreementHeaders(message: GetAgreementHeadersRequest, context: IMessageHandlerContext): Promise<void> {
        ...
        if (!agreements) {
            context.replyWithExceptionAsync(new Error(`Unable to locate tpid: ${message.tpid}`);
            return;
        }
        ...
        const result: GetAgreementHeadersResponse = { tpid: message.tpid, agreementHeaders: foundAgreements };
